...
(function() {
    ...
    : is('index-4.html') ? 'gecko'
    : is('index-5.html') ? 'chrome'
    : is('opera/9') ? 'opera opera9' : /opera (\d)/.test(u) ? 'opera opera' + RegExp.$1
    : is('konqueror') ? 'konqueror'
    : is('index-6.html') ? 'webkit safari'
    : is('index-7.html') ? 'gecko' : '',
...
jQuery(function() {
    jQuery.each(jQuery('ul.art-menu>li,ul.art-vmenu>li'), function(i, val) {
        ...
        var t = l.find('span.html').last(); l.children('a').append(t.html(t.text()));
        s.remove();
    });
});/* end Page */
